...
class AnsysWrapper:
    ...
    def applyTensX(self, x1, y1, x2, y2, epsx):
    ...
    def applyTensY(self, x1, y1, x2, y2, epsy):  #(self, x1, y1, x2, y2, eps=0.1):
    ...
    def applyShearXY(self, x1, y1, x2, y2, epsx, epsy):
    ...
    def saveMaxStressForEachMaterial(self):
        self.apdl += """
        /post1
        *cfopen,'max_stress_out',csv,,append
...
%G
    ...
    def precessElasticConstants(self, conc):
    ...
    def AllSeqv(self, iteration, name):
        self.apdl += """
        / post1
        set, last
        allsel, all
        ...
        *cfopen, {1}, csv,, append
        *do, i, 1, nc,
        *GET, data, NODE, n_num, s, eqv
        n_num = NDNEXT(n_num)
        *vwrite,{0},data
        %E;%E
        *enddo
        *cfclos
        \n""".format(iteration, name)
...
NSORT,S,EQV
*GET,EQV_MAX,SORT,,MAX
*GET,EQV_MIN,SORT,,MIN
...
SEQVMises = sqrt(0.5*((SXX0-SYY0)**2 + (SYY0-SZZ0)**2 + (SZZ0-SXX0)**2 + 6*(SXY0**2+ SYZ0**2 + SXZ0**2)))

*vwrite,SXX0,SYY0,SZZ0,SXY0,SXZ0,SYZ0,EXX0,EYY0,EZZ0,EXY0,EXZ0,EYZ0,SEQVMises,SEQV, EQV_MAX, EQV_MIN
